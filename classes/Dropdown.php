<?php

namespace app\classes;

use yii\base\InvalidConfigException;
use yii\bootstrap5\Html;
use yii\helpers\ArrayHelper;

class Dropdown extends \yii\bootstrap5\Dropdown
{
    public function init()
    {
        Html::addCssClass($this->options, ['widget' => 'dropdown-menu dropdown-menu-dark']);
    }

    protected function renderItems(array $items, array $options = []): string
    {
        $lines = [];
        foreach ($items as $key =>$item) {
            if (is_string($item)) {
                $lines[] = ($item === '-')
                    ? Html::tag('hr', '', ['class' => 'dropdown-divider'])
                    : $item;
                continue;
            }
            if (isset($item['visible']) && !$item['visible']) {
                continue;
            }
            if (array_key_exists('label', $item)) {
                $encodeLabel = $item['encode'] ?? $this->encodeLabels;
                $label = $encodeLabel ? Html::encode($item['label']) : $item['label'];
                $itemOptions = ArrayHelper::getValue($item, 'options', []);
                $linkOptions = ArrayHelper::getValue($item, 'linkOptions', []);
                $active = ArrayHelper::getValue($item, 'active', false);
                $disabled = ArrayHelper::getValue($item, 'disabled', false);

                Html::addCssClass($linkOptions, ['widget' => 'dropdown-item']);
                if ($disabled) {
                    ArrayHelper::setValue($linkOptions, 'tabindex', '-1');
                    ArrayHelper::setValue($linkOptions, 'aria.disabled', 'true');
                    Html::addCssClass($linkOptions, ['disable' => 'disabled']);
                } elseif ($active) {
                    ArrayHelper::setValue($linkOptions, 'aria.current', 'true');
                    Html::addCssClass($linkOptions, ['activate' => 'active']);
                }

                $url = array_key_exists('url', $item) ? $item['url'] : null;
                if (empty($item['items'])) {
                    if ($url === null) {
                        $content = Html::tag('h6', $label, ['class' => 'dropdown-header']);
                    } else {
                        $content = Html::a($label, $url, $linkOptions);
                    }
                    if (!empty($item['tag'])){
                        $content = Html::beginTag('div',['class' => 'col-lg-12 float-start text-center']);
                        $content .= Html::tag($item['tag'],$label,$itemOptions);
                        $content .= Html::endTag('div');
                    }
                    $lines[] = $content;
                } else {
                    $submenuOptions = $this->submenuOptions;
                    if (isset($item['submenuOptions'])) {
                        $submenuOptions = array_merge($submenuOptions, $item['submenuOptions']);
                    }
                    //Html::addCssClass($submenuOptions, ['widget' => 'dropdown-submenu dropdown-menu']);
                    Html::addCssClass($linkOptions, ['toggle' => 'dropdown-toggle']);
                    $lines[] = Html::beginTag('div', array_merge_recursive(['class' => ['dropdown'], 'aria' => ['expanded' => 'false']], $itemOptions));
                    $lines[] = Html::a($label, $url, array_merge_recursive([
                        'data' => ['bs-toggle' => 'dropdown'],
                        'aria' => ['expanded' => 'false'],
                        'role' => 'button',
                    ], $linkOptions));
                    $lines[] = static::widget([
                        'items' => $item['items'],
                        'options' => $submenuOptions,
                        'submenuOptions' => $submenuOptions,
                        'encodeLabels' => $this->encodeLabels,
                    ]);
                    $lines[] = Html::endTag('div');
                }
            } else {
                $dropdownItem='';
                if (!($key%2)){
                    $dropdownItem=Html::beginTag('div',['class' => 'dropdown-row']);
                }
                $options = ['class' => ['widget' => 'dropdown-menu dropdown-menu-dark big-dropdown row']];
                $bigDropdownClasses = 'sub-dropdown col-lg-6';
                $dropdownItem .= $this->renderItems($item, ['class' => ['widget' => $bigDropdownClasses]]);
                if (($key%2) || $key === (count($items) -1)){
                    $dropdownItem .= Html::endTag('div');
                }
                $lines[] = $dropdownItem;
            }
        }
        return Html::tag('div', implode("\n", $lines), $options);
    }
}